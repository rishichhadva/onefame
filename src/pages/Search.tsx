        ...
        <section className="bg-gradient-hero border-b py-12">
          <div className="container">
            <h1 className="text-3xl md:text-4xl font-bold mb-6 text-center">
              Find Your Perfect <span className="bg-gradient-primary bg-clip-text text-transparent">Provider</span>
            </h1>
          ...
          <div className="container">
            <div className="flex flex-col md:flex-row gap-8 items-start justify-center">
              ...
              <div className="w-full md:max-w-xs bg-white rounded-2xl shadow-lg p-6 h-fit md:sticky md:top-24">
                ...
                <button onClick={handleApplyFilters} className="w-full px-5 py-2 rounded-md bg-gradient-to-r from-blue-500 to-cyan-400 text-white font-semibold shadow hover:from-blue-600 hover:to-cyan-500 transition mb-2">Apply Filters</button>
                <button onClick={handleReset} className="w-full px-5 py-2 rounded-md bg-gray-200 font-semibold shadow hover:bg-blue-100 transition mb-2">Reset</button>
              </div>
              {/* Main Search & Results */}
              <div className="flex-1">
                <div className="bg-white p-8 rounded-2xl shadow-lg mb-8">
                  <div className="flex gap-4 items-center mb-4" style={{ alignItems: 'center', marginBottom: 0, marginTop: '12px' }}>
                    <input
                      ...
                      className="w-full px-6 py-3 border-2 border-blue-200 rounded-2xl focus:outline-none focus:border-blue-500"
                    />
                    <button
                      onClick={handleSearch}
                      className="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-r from-blue-500 to-cyan-400 shadow hover:from-blue-600 hover:to-cyan-500 transition"
                      aria-label="Search"
                      style={{ marginTop: '2px' }}
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                  {services.length === 0 ? (
                    ...
                    services.map(offering => (
                      <div key={offering.id} className="bg-white rounded-xl shadow p-4 mb-4 cursor-pointer" onClick={() => navigate(`/profile?id=${offering.id}`)}>
                        <h3 className="text-xl font-bold text-blue-700">{offering.name?.replace(/Service/g, 'Offering') || offering.name}</h3>
                        <div className="text-gray-700">Provider: {offering.provider}</div>
                        <div className="text-gray-500 text-sm">{offering.category} | {offering.location}</div>
                        <div className="text-blue-600 font-bold">₹{offering.price}</div>
                        <div className="text-yellow-500">Rating: {offering.rating} ★</div>
                        <button className="mt-2 px-5 py-2 rounded-md bg-gradient-to-r from-blue-500 to-cyan-400 text-white font-bold shadow hover:from-blue-600 hover:to-cyan-500 transition">View Profile</button>
                      </div>
                    ))
                  )}
                </div>
                ...
                {selectedProfile && (
                  <div className="bg-white rounded-2xl shadow-lg p-8 max-w-xl mx-auto">
                    ...
                    {selectedProfile.services && <div className="mb-2"><strong>Offerings:</strong> {selectedProfile.services}</div>}
                    <div className="mt-4 flex gap-4">
                      ...
                      <button className="px-4 py-2 bg-green-600 text-white rounded-full font-bold shadow hover:bg-green-700 transition">Get Started</button>
                    </div>
                  </div>
                )}
              </div>
