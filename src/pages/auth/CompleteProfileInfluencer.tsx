...
const CompleteProfileInfluencer = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setPopup('');
    if (!bio || !interests || !skills || !location || !experience || !socials) {
    ...
    try {
      ...
      const res = await fetch(apiUrl("/api/profile"), {
        ...
        body: JSON.stringify({ bio, interests, skills, location, experience, socials })
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-pink-50">
      <div className="w-full max-w-md p-10 space-y-8 bg-white rounded-2xl shadow-lg">
        <h2 className="text-3xl font-extrabold text-pink-700 text-center mb-4">Complete Your Influencer Profile</h2>
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <input type="text" value={socials} onChange={e => setSocials(e.target.value)} placeholder="Socials" required className="w-full px-4 py-2 border-2 border-pink-200 rounded-xl focus:outline-none focus:border-pink-500" />
...
export default CompleteProfileInfluencer;
